...
% 
 Screen('Preference', 'ConserveVRAM', 4096);
            ...
            SetResolution(screenID, 1920, 1080, 480);
            % [win, rect] = PsychImaging('OpenWindow', screenID, [0 0 0]);% Opens default PTB window on desired display with a black background
            % Open the window with the fast-clearing engine fix (the '2' at the end)
            
            % =========================================================================
            % THE ENGINE FIX: Initialize and configure the modern imaging pipeline
            % =========================================================================
            PsychImaging('PrepareConfiguration');

            % Open the window using PsychImaging instead of Screen('OpenWindow')
            ...
            screenFlips = []; %Array of last 20 flip timestamps
            avgFPS_20 = 0; %avg from screenFlips
            framesPerUpdate = max(1,round((frameDuration)/ifi)); %number of frames per flip
            frameCounter = 0;
            motionCounter = 1;
            ...
            % Setup experimental screen
            % data = struct('SubjectNum',[],'blockNum',[],'frameRate',[],'TimeStamp',[],'onTarget',[],'Txi',[],'Tyi',[],'Tzi',[],'Cxi',[],'Cyi',[],'Czi',[]);
            data = struct([]);

            visibleDots = [xCenter; yCenter];   % dummy dot so variable always exists
            visibleSizes = 1;
            ...
            while true
                ...
                pathSampleRate = 480;
                pathStep = round(pathSampleRate / frameRate);
                ...
                data(idx).TimeStamp = vbl;
                data(idx).Txi = screen_targetX;
                data(idx).Tyi = screen_targetY;
                data(idx).Tzi = distance;
                ...
                [vbl,stimulusOnset,flipEnd,missed] = Screen('Flip',win,previousVbl+frameDuration-0.5*ifi);


                % vbl = Screen('Flip', win);
                flipInterval = vbl-previousVbl;
